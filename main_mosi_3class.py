...
# ==========================================
# 3. CMU-MOSI 数据集（三分类，保留 Neutral）
# ==========================================
# Negative=0, Neutral=1, Positive=2
LABEL_MAP_3CLASS = {'Negative': 0, 'Neutral': 1, 'Positive': 2}

class MOSIDataset3Class(Dataset):
    def __init__(self, h5_t_path, h5_a_path, h5_v_path, meta_path, split='train', augment=True):
        ...
        self.label_map = LABEL_MAP_3CLASS
        ...
        # 保留全部三类
        df = df[df['annotation'].isin(LABEL_MAP_3CLASS.keys())].reset_index(drop=True)
        ...
        neu  = np.sum(self.labels == 1)
        pos  = np.sum(self.labels == 2)
        print(f"[{split.upper()}] MOSI 3-Class Dataset Loaded! Valid: {self.num_samples} "
...
def train():
    ...
    train_dataset = MOSIDataset3Class(h5_t_path, h5_a_path, h5_v_path, meta_path, split='train')
    valid_dataset = MOSIDataset3Class(h5_t_path, h5_a_path, h5_v_path, meta_path, split='valid')

    # 类别均衡采样（对 Neutral 少数类尤其重要）
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if valid_acc > best_valid_acc:
            ...
            torch.save(model.state_dict(), 'best_model_mosi_3class.pth')
