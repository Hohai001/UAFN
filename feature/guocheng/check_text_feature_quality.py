...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Quality checks for text features stored in an HDF5 file."
    )
    parser.add_argument(
        "--meta-path",
        type=str,
    ...
    parser.add_argument(
        "--h5-path",
        type=str,
        default="text_features.h5",
        help="Path to feature HDF5 file.",
    )
    parser.add_argument(
        "--id-col",
        type=str,
...
def build_sample_keys(df: pd.DataFrame, id_col: str):
    ...
    # Keep exactly the same key rule used in sims_v2_text_to_h5.py.
