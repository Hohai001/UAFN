...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Quality checks for visual features stored in an HDF5 file."
    )
    parser.add_argument(
        "--meta-path",
        type=str,
    ...
    parser.add_argument(
        "--h5-path",
        type=str,
        default="video_features.h5",
        help="Path to visual feature HDF5 file.",
    )
    parser.add_argument(
        "--id-col",
        type=str,
...
def build_sample_keys(df: pd.DataFrame, id_col: str):
    ...
    # Keep exactly the same key rule used in extract_sims_videoswin.py.
...
def main():
    ...
    x = (
        ...
        if vectors
        else np.empty((0, args.expected_dim), dtype=np.float32)
    )
