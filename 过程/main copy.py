...
class UncertaintyAwareFusion(nn.Module):
    def __init__(self, dim_t, dim_a, dim_v, num_classes, dropout=0.3):
...
class MultiModalEvidentialNetwork(nn.Module):
    def __init__(self, dim_t=768, dim_a=768, dim_v=768, num_classes=3, dropout=0.3):
        ...
        # 动态自适应融合模块
        self.fusion = UncertaintyAwareFusion(dim_t, dim_a, dim_v, num_classes, dropout)
...
class CHSIMSDataset(Dataset):
    def __init__(self, h5_t_path, h5_a_path, h5_v_path, meta_path, split='train'):
        ...
        # 使用原始行号生成key，与特征文件保持一致
        self.sample_keys = [f"{vid.replace('/', '_')}_{idx}" for vid, idx in zip(df_split['video_id'].astype(str), df_split['original_idx'])]
        self.labels = df_split['annotation'].map(self.label_map).values
