...
class EvidentialModule(nn.Module):
    def __init__(self, input_dim, num_classes, dropout=0.5, hidden_dim=64):  # hidden_dim: 128 -> 64
...
# ==========================================
# 3. 数据集读取（二分类：丢弃 Neutral）
# ==========================================
BINARY_LABEL_MAP = {'Negative': 0, 'Positive': 1}

class CHSIMSDatasetBinary(Dataset):
    def __init__(self, h5_t_path, h5_a_path, h5_v_path, meta_path, split='train', augment=True):
        ...
        valid_keys, valid_labels, missing_count = [], [], 0
        ...
        print(f"[{split.upper()}] Binary Dataset Loaded! Valid: {self.num_samples} "
              f"(Skipped {missing_count} missing). Label map: {self.label_map}")
        print(f"  -> Negative: {np.sum(self.labels == 0)}, Positive: {np.sum(self.labels == 1)}")
    ...
    def _augment_feature(self, feat, noise_level=0.05):  # 0.02 -> 0.05
...
def train():
    ...
    #   hidden_dim:   128   -> 64  (EvidentialModule)
    #   epochs:       50    -> 60  (给更充分的验证窗口)
    print("=" * 50)
    print("[v2] dropout=0.58 | weight_decay=1e-2 | noise=0.05 | hidden_dim=64")
    print("=" * 50)
    ...
    model = MultiModalEvidentialNetwork(
        ...
        dropout=0.58,        # 0.45 -> 0.58
        min_weight=0.05,
        temperature=0.5
    ).to(DEVICE)

    train_dataset = CHSIMSDatasetBinary(h5_t_path, h5_a_path, h5_v_path, meta_path, split='train')
    valid_dataset = CHSIMSDatasetBinary(h5_t_path, h5_a_path, h5_v_path, meta_path, split='valid')
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=1e-2)  # 5e-3 -> 1e-2
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if valid_epoch_acc > best_valid_acc:
            ...
            torch.save(model.state_dict(), 'best_model_binary_v2.pth')
