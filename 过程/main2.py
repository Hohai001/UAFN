...
class UncertaintyAwareFusion(nn.Module):
    def __init__(self, dim_t, dim_a, dim_v, num_classes, dropout=0.5, min_weight=0.01, temperature=0.05):
        ...
        # 🌟 2. 终极突破：Transformer 跨模态交互层
        # 让 T, A, V 互相看到对方，理解“阴阳怪气”等反转情感
    ...
    def forward(self, h_t, h_a, h_v, u_t, u_a, u_v):
        ...
        # 2. 降维并增加 Sequence 维度
        proj_t = self.proj_t(h_t).unsqueeze(1) # [batch, 1, 128]
        proj_a = self.proj_a(h_a).unsqueeze(1)
        proj_v = self.proj_v(h_v).unsqueeze(1)
        
        # 3. Transformer 围桌开会
        stacked_feat = torch.cat([proj_t, proj_a, proj_v], dim=1) # [batch, 3, 128]
        interacted_feat = self.transformer(stacked_feat)
        
        # 散会，拿回各自交流后的新特征
        feat_t_new = interacted_feat[:, 0, :]
        feat_a_new = interacted_feat[:, 1, :]
        feat_v_new = interacted_feat[:, 2, :]
        
        # 4. 加权拼接与分类
        H_fused = torch.cat([w_t * feat_t_new, w_a * feat_a_new, w_v * feat_v_new], dim=1)
...
class CHSIMSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            # Modality Dropout (各 15% 概率)
            if torch.rand(1).item() < 0.15: feat_t = torch.zeros_like(feat_t)
            if torch.rand(1).item() < 0.15: feat_a = torch.zeros_like(feat_a)
            if torch.rand(1).item() < 0.15: feat_v = torch.zeros_like(feat_v)
...
def train():
    ...
    # 注意：如果你用的是人脸特征，把 dim_v=768 改成 dim_v=512！
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch_idx, (h_t, h_a, h_v, labels) in enumerate(train_loader):
            ...
            # 🌟 终极突破：非对称模态偏置 (文本全责，音视频辅助)
            loss_aux = 1.0 * loss_t + 0.5 * loss_a + 0.5 * loss_v
